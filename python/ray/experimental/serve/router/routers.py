...
@ray.remote
class JanusRouter:
    """JanusRouter is a deadline aware router that implements the Janus Reactive Controller.

    It will adjust the replication factor to handle query arrival rate changes
    in order to meet a given latency SLO.

    It takes into consideration the deadline attached to each query. It will
    reorder incoming query based on their deadlines.
    """

    def __init__(self, router_name, init_config):
        # Runtime Data
        self.query_queues: Dict[str, PriorityQueue] = defaultdict(
            PriorityQueue)
        self.running_queries: Dict[ray.ObjectID, ray.actor.ActorHandle] = {}
        ...
        self.traffic_envelope = json.loads(init_config)["TrafficEnvelope"]
        self.model_data = json.loads(init_config)["ModelData"]
        self.lambda_val = json.loads(init_config)["Lambda"]
